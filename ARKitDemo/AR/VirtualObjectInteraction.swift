...
class VirtualObjectInteraction: NSObject, UIGestureRecognizerDelegate {
    ...
    var trackedObject: SCNReferenceNode? {
        didSet {
    ...
    init(sceneView: ARSCNView, viewController: ARViewController) {
    ...
    func updatedTrackingPosition(for object: SCNReferenceNode, from gesture: UIPanGestureRecognizer) -> CGPoint {
    ...
    func didRotate(_ gesture: UIRotationGestureRecognizer) {
        ...
        let ro = trackedObject?.childNodes.first!.eulerAngles.y
        if let ro = ro {
            let neRo = ro - Float(gesture.rotation)
            
            trackedObject?.childNodes.first!.eulerAngles.y = neRo
        }
    ...
    func didTap(_ gesture: UITapGestureRecognizer) {
        ...
        if let tappedObject = self.virtualObject(sceneView:sceneView,at: touchLocation) {
    ...
    func virtualObject(sceneView:ARSCNView, at point: CGPoint) -> SCNReferenceNode? {
    ...
    /** A helper method to return the first object that is found under the provided `gesture`s touch locations.
     ...
     gestures, the method makes it more likely that the user touch affects the intended object.
      - Tag: TouchTesting
    */
    private func objectInteracting(with gesture: UIGestureRecognizer, in view: ARSCNView) -> SCNReferenceNode? {
        for index in 0..<gesture.numberOfTouches {
            let touchLocation = gesture.location(ofTouch: index, in: view)
            
            // Look for an object directly under the `touchLocation`.
            if let object = virtualObject(sceneView:sceneView,at: touchLocation) {
                return object
            }
        }
        ...
        if let center = gesture.center(in: view) {
            return virtualObject(sceneView:sceneView,at: center)
        }
    ...
    func translate(_ object: SCNReferenceNode, basedOn screenPos: CGPoint) {
...
//            }
    ...
    func setDown(_ object: SCNReferenceNode, basedOn screenPos: CGPoint) {
//        object.stopTrackedRaycast()
//
